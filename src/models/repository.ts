...
export function getPersona(id: number): KOLPersona | undefined {
  const row = getDatabase().prepare("SELECT * FROM personas WHERE id = ?").get(id) as Record<string, unknown> | undefined;
  if (!row) return undefined;
  return deserializePersona(row);
}

export function getPersonaByName(name: string): KOLPersona | undefined {
  const row = getDatabase().prepare("SELECT * FROM personas WHERE name = ?").get(name) as Record<string, unknown> | undefined;
  if (!row) return undefined;
  return deserializePersona(row);
}

export function listPersonas(): KOLPersona[] {
  const rows = getDatabase().prepare("SELECT * FROM personas ORDER BY created_at DESC").all() as Record<string, unknown>[];
  return rows.map(deserializePersona);
}

export function createPersona(data: Omit<KOLPersona, "id" | "createdAt" | "updatedAt">): KOLPersona {
...
export function getTemplate(id: number): ContentTemplate | undefined {
  const row = getDatabase().prepare("SELECT * FROM content_templates WHERE id = ?").get(id) as Record<string, unknown> | undefined;
  if (!row) return undefined;
...
export function listTemplates(personaId?: number): ContentTemplate[] {
  if (personaId) {
    const rows = getDatabase().prepare("SELECT * FROM content_templates WHERE persona_id = ? ORDER BY created_at DESC").all(personaId) as Record<string, unknown>[];
    return rows.map(deserializeTemplate);
  }
  const rows = getDatabase().prepare("SELECT * FROM content_templates ORDER BY created_at DESC").all() as Record<string, unknown>[];
  return rows.map(deserializeTemplate);
}

export function createTemplate(data: Omit<ContentTemplate, "id" | "createdAt" | "updatedAt">): ContentTemplate {
  const stmt = getDatabase().prepare(`
  ...
  `);
  const result = stmt.run(
    data.personaId, data.name, data.type, data.platform,
    data.systemPrompt, data.userPromptTemplate,
    data.maxTokens, data.temperature, JSON.stringify(data.hashtags),
  );
...
export function getScheduledPost(id: number): ScheduledPost | undefined {
  const row = getDatabase().prepare("SELECT * FROM scheduled_posts WHERE id = ?").get(id) as Record<string, unknown> | undefined;
  if (!row) return undefined;
  return deserializePost(row);
}

export function listScheduledPosts(filter?: { status?: string; personaId?: number }): ScheduledPost[] {
  let sql = "SELECT * FROM scheduled_posts WHERE 1=1";
  const params: unknown[] = [];
  if (filter?.status) { sql += " AND status = ?"; params.push(filter.status); }
  if (filter?.personaId) { sql += " AND persona_id = ?"; params.push(filter.personaId); }
  sql += " ORDER BY scheduled_at ASC";
  const rows = getDatabase().prepare(sql).all(...params) as Record<string, unknown>[];
  return rows.map(deserializePost);
}

export function createScheduledPost(data: Omit<ScheduledPost, "id" | "postedAt" | "createdAt" | "updatedAt">): ScheduledPost {
  const stmt = getDatabase().prepare(`
    INSERT INTO scheduled_posts (template_id, persona_id, platform, status, content, scheduled_at, metadata)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  const result = stmt.run(
    data.templateId ?? null, data.personaId, data.platform,
    data.status, data.content ?? null, data.scheduledAt ?? null,
    data.metadata ? JSON.stringify(data.metadata) : null,
  );
  return getScheduledPost(Number(result.lastInsertRowid))!;
}

export function updatePostStatus(id: number, status: string, content?: string): void {
  if (content) {
    getDatabase().prepare("UPDATE scheduled_posts SET status = ?, content = ?, posted_at = datetime('now'), updated_at = datetime('now') WHERE id = ?").run(status, content, id);
  } else {
    getDatabase().prepare("UPDATE scheduled_posts SET status = ?, updated_at = datetime('now') WHERE id = ?").run(status, id);
  }
}

export function updateScheduledPost(id: number, data: Partial<Omit<ScheduledPost, "id" | "createdAt" | "updatedAt">>): ScheduledPost | undefined {
  const updates: string[] = [];
  const params: unknown[] = [];

  if (data.templateId !== undefined) { updates.push("template_id = ?"); params.push(data.templateId); }
  if (data.personaId !== undefined) { updates.push("persona_id = ?"); params.push(data.personaId); }
  ...
  if (data.scheduledAt !== undefined) { updates.push("scheduled_at = ?"); params.push(data.scheduledAt); }
  ...
  getDatabase().prepare(`UPDATE scheduled_posts SET ${updates.join(", ")} WHERE id = ?`).run(...params);
  return getScheduledPost(id);
}

export function deleteScheduledPost(id: number): boolean {
  const result = getDatabase().prepare("DELETE FROM scheduled_posts WHERE id = ?").run(id);
  return result.changes > 0;
}

// === Analytics Repository ===
export function recordAnalytics(data: Omit<AnalyticsEvent, "id">): AnalyticsEvent {
  const stmt = getDatabase().prepare(`
    INSERT INTO analytics_events (post_id, platform, event_type, count, recorded_at)
    VALUES (?, ?, ?, ?, ?)
  `);
  const result = stmt.run(data.postId, data.platform, data.eventType, data.count, data.recordedAt);
  const row = getDatabase().prepare("SELECT * FROM analytics_events WHERE id = ?").get(Number(result.lastInsertRowid)) as Record<string, unknown>;
...
export function getDashboardStats(): DashboardStats {
  ...
  const totalPostsRow = db.prepare("SELECT COUNT(*) AS c FROM scheduled_posts").get() as { c: number };
  const totalPosts = totalPostsRow.c;

  const statusRows = db.prepare("SELECT status, COUNT(*) AS c FROM scheduled_posts GROUP BY status").all() as Array<{ status: string; c: number }>;
  ...
  const postsTodayRow = db.prepare(
  ...
  const personasRow = db.prepare("SELECT COUNT(*) AS c FROM personas").get() as { c: number };
  const templatesRow = db.prepare("SELECT COUNT(*) AS c FROM content_templates").get() as { c: number };
...
export function getAnalyticsByPost(postId?: number): PostAnalyticsGroup[] {
  ...
  `).all(...params) as Array<{ post_id: number; platform: string; event_type: string; total: number }>;
...
export function getRecentPosts(limit?: number): RecentPostWithAnalytics[] {
  ...
  const postRows = db.prepare(
    "SELECT * FROM scheduled_posts ORDER BY created_at DESC LIMIT ?"
  ...
  `).all(...posts.map(p => p.id)) as Array<{ post_id: number; event_type: string; total: number }>;

  const analyticsByPost = new Map<number, { byEventType: EngagementByType; total: number; platform: PostAnalyticsGroup["platform"] | null }>();
  for (const row of analyticsRows) {
    let entry = analyticsByPost.get(row.post_id);
    if (!entry) {
      entry = {
        byEventType: { impression: 0, like: 0, comment: 0, share: 0, retweet: 0, click: 0 },
        total: 0,
        platform: null,
      };
